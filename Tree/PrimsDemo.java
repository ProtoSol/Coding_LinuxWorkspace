    ...
    void buildMST(int [][] graph){
        ...
        for(int i = 0 ; i<V-1; i++){
            ...
            for(int j = 0; j<V; j++){
                 ...
                 */
                // This for loop will select the Minimum value from the Array and update the parameters.
                if(graph[U][j]!=0 && !mst[j] && graph[U][j]<weight[j]){
                    weight[j] = graph[U][j]; // Update the Weight from Infinity to the Smaller Weight found.
